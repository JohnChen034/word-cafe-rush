...
export class ParticleFactory {
  ...
  coinBurst(x: number, y: number, count = 12): void {
    for (let index = 0; index < count; index += 1) {
      const coin = this.scene.add.image(x, y, "juice-coin").setDepth(430).setScale(0.8);
  ...
  sparkleBurst(x: number, y: number, count = 9): void {
    for (let index = 0; index < count; index += 1) {
      const sparkle = this.scene.add.image(x, y, "juice-spark").setDepth(440);
  ...
  stampPuff(x: number, y: number): void {
    for (let index = 0; index < 8; index += 1) {
      const puff = this.scene.add.image(x, y, "juice-puff").setDepth(410).setScale(0.7);
  ...
  ring(x: number, y: number, color = 0xe5a940, radius = 36): void {
    const circle = this.scene.add.circle(x, y, radius, color, 0).setStrokeStyle(4, color, 0.9).setDepth(420);
    this.scene.tweens.add({
      targets: circle,
      scale: 1.65,
      alpha: 0,
